...
class web_scracher_tw_manhuagui_com(web_scracher):
    def __init__(self , web:str, preval:str ):
        self._web = web
        self._preval = preval
        pass
    def search_update(self):
...
class web_scracher_www_manhuaren_com(web_scracher):
    ...
    def search_title(self):
        for link in self._soup.findAll( name= 'title'):
            title = link.text
        self._title = title
def check_manga_updating_state( web:str,preval:str ) -> Union( manga_failed ,manga_nonupdated , manga_updated ):
